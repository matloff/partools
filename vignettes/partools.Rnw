...
the years following its birth in 2006.
Yet it was clear from the start that Hadoop had major shortcomings in
performance, and recently these are being much more seriously
discussed.\footnote{See for example ``The Hadoop Honeymoon is Over,''
{\it https://www.linkedin.com/pulse/hadoop-honeymoon-over-martyn-jones}}
This has resulted in a new platform, Spark, gaining popularity.  As with
...
thus speeding up computation.  In addition, this approach helps deal
...
alternative to Hadoop/Spark, we informally refer to {\bf partools} as
Snowdoop.
...
desired operations --- and avoiding operations that collect data from
the worker nodes to the manager node.}
...
Alchemy (SA) method ({\it Parallel Computation for Data Science}, N.
Matloff, Chapman and Hall, 2015) to parallelize statistical
operations.\footnote{More detailed presentation in forthcoming paper in
the {\it Journal of Statistical Software}.}  The idea is simple:  Apply
the given estimator to each chunk in the distributed object, and average
...
averaging process.  Consider for instance the use of LASSO for
...
classification context, voting is used.
...
cluster in the sense of the R {\bf parallel} package.  carefully,
though, that we should not expect a 16-fold speedup. In the world of
parallel computation, one usually gets of speedups of considerably less
than $n$ for a platform of $n$ computational entities, in this case with
$n = 16$.  Indeed, one is often saddened to find that the parallel
version is actually {\it slower} than the serial one!
...
i.e.\ the eighth one, is minuscule is statistically  0.  Again, SA gives
...
\section{Statistical/Machine Learning Example}

Here we apply random forests to the UC Irvine forest cover type data,
which has 590,000 records of 55 variables.  The last variable is the co
ver type, coded 1 through 7.  The machine used was quad core with
hyperthreading level of 2.  (Some output not shown.)
...
> cvr <-
    read.csv('~/Research/DataSets/ForestCover/CovTypeFull.csv',header=F)
> cvr$V55 <- as.factor(cvr$V55)  # rpart requires
...
> system.time(fit <- caclassfit(cls,'rpart(V55 ~ .,data=trn,xval=25)'))
   user  system elapsed
  0.126   0.005  25.294
> # system.time(predout <- caclasspred(fit,tst,10,type='class'))
...
So, SA more than tripled the fitting speed, with the same accuracy.
