...
Suppose we want to see all possible values for the {\bf ArrTime, DepTime }
variables. It's important to note that we reduce locally on each 

\begin{verbatim}
#  2 nodes produced errors; first error: replacement has 3428517 rows, data
#  has 3504864

distribunique(cls, "yr2008")

alltimes = clusterEvalQ(cls, unique(c(yr2008$ArrTime, yr2008$DepTime)))

alltimes = sort(unique(c(alltimes[[1]], alltimes[[2]])))
\end{verbatim}
...
# Verify locally that code does the right thing
minute(c(1L, 20L, 301L, 1937L))
# [1]  1 20  1 37
...
Alternatively one could write this function to do global assignment within
the workers using {\bf <<- }, but this is not as clean.
