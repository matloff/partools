...
Through the use of the R 'parallel' package, these tools can be used
either on multicore machines or clusters, such as in the cloud.  With
the latter, a very high degree of parallelism is potentially possible.
...
manager node, then distributed it to the nodes via
\textbf{distribsplit()}.
