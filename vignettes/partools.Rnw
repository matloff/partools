...
We may wish to gather together for in-depth analysis all the records
have 8 or more NA values.  In the nonparallel context, it would take
some time:
...
system.time(rowsf <- with(yr2008,yr2008[DayOfWeek==1 & DepTime > 1800,]))
system.time(rowsf <- with(yr2008,yr2008[DayOfWeek > 1 & DayOfWeek < 7 & DepTime > 1800 & Distance < 1000,]))
