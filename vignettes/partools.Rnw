...
The function {\bf filesplitrand()} was used instead of {\bf filesplit()}
to construct the distributed file, in order to randomize the placement
of the records of {\bf yr2008} across cluster nodes.  The advantage of
randomization will be explained shortly.

In order to run timing comparisons, the full file was also read into
memory at the cluster manager:

\begin{verbatim}
> yr2008 <- read.csv("2008.csv")
\end{verbatim}
...
\begin{verbatim}
> system.time(print(distribagg(cls, c("DayOfWeek","Month","Cancelled"),
...
...
   user  system elapsed  
260.372   0.284 260.732  
\end{verbatim}

So, the results of {\bf distribagg()} did indeed match those of {\bf
aggregate()}, but more than 11 times faster!
